...
class PoseidonModelTester extends FreeSpec with ChiselScalatestTester {
    "Software full round should work" in {
        ...
        assert(PoseidonModel.roundFunction(p, v, true, 0) == Seq("1434672155438326909468395403595442108100131599008064476955240820749368388332", "4833344523483475792495452519536041633104809104825374711308215753935826116910", "1220913282961632239274840494584456300408222746440926547802646300125394075962"))

    }
    "Software half round should work" in {
        val v = ArrayBuffer(BigInt(0),BigInt(1),BigInt(2))
        val p = PoseidonParams(r = 64, c = 64, Rf = 8, Rp = 57, alpha = 5)
        assert(PoseidonModel.roundFunction(p, v, false, 0) == Seq("21038416477568674046849954456272533174388763670098315194144749698091882799577", "19568226743792614801321798830704698149002855748864219953241937962343579857238", "10804600181627836747048859459573044908538498894105223567676355450893040239813"))
